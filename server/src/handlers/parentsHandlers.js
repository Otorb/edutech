...
export async function updateWithImageHandler(req, res) {
  try {
    ...
    const resultParent = await updateWithImage(id, { name, lastName, email, address, phone, photo: image });
    res.status(200).json({ message: "Update data Parent + Image", resultParent });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
}
...
export async function updateWithoutImageHandler(req, res) {
  try {
    ...
    const resultParent = await updateWithoutImage(id, { name, lastName, email, address, phone });
